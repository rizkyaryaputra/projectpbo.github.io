...
class Sewa(ManajemenSistem):
	...
	def ambil(self):
		self.query = f"SELECT a.id_sewa, b.nama_kendaraan, c.durasi, d.nama, a.tanggal_pinjam, a.tanggal_kembali, a.status_peminjaman FROM {self.__namaTabel} a INNER JOIN {self.__join1} b using(id_kendaraan) INNER JOIN {self.__join2} c using(id_waktu) INNER JOIN {self.__join3} d using(id_user)"
		hasil = self.read()
		...
		if(hasil):
			...
			for x in range(0, len(hasil)):		
				...
				print(f"{x+1}. (ID: {hasil[x][0]}) {hasil[x][3]} | Kendaraan: {hasil[x][1]} | Durasi: {hasil[x][2]} hari | Tanggal Pinjam: {hasil[x][4]} | Tanggal Kembali: {hasil[x][5]} | Status: {status}")
		else:
			print("=== Riwayat Transaksi ===")
			print("Tidak ada data")
	...
	def ambilTransaksiUser(self, inputan):
		self.query = f"SELECT a.id_sewa, b.nama_kendaraan, c.durasi, d.nama, a.tanggal_pinjam, a.tanggal_kembali, a.status_peminjaman FROM {self.__namaTabel} a INNER JOIN {self.__join1} b using(id_kendaraan) INNER JOIN {self.__join2} c using(id_waktu) INNER JOIN {self.__join3} d using(id_user) WHERE id_user = ?"
		self.data = (inputan,)
...
def riwayatSewa():
	...
	if(menu == 1):
		...
		if(cek_data):
			...
			if(data_ada and data_ada[4] == "1"):
				input1 = input(f"Apakah transaksi dengan id {inputan} telah melakukan transfer? (1 = Ya | 0 = Tidak): ")
				data = (input1, inputan)

				if(input1 == "1"):
					bersihkanCmd()
					Sewa().prosesTransfer(data)
				else:
					pass
			elif(data_ada and data_ada[4] == "0"):
